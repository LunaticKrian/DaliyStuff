...
- **桌面客户端** — macOS 原生壳（Tauri 2 瘦客户端）：首屏接驳服务器、钥匙串持久登录、托盘 + 全局快捷键、设备会话管理（预编译包见仓库 Releases，开发/构建见下方「桌面客户端」节）
...
| 桌面端 | Tauri 2 + Rust（瘦客户端，复用 `web/`，令牌存 OS Keychain） |
| 认证 | JWT + 会话化（refresh 轮换 / 复用检测 / 设备会话） |
| 图表 | ECharts 6 |
| 风格 | NES.css + 自定义像素风组件 |
...
│   │   ├── utils/         # 工具函数（格式化、导出、计算、平台抽象、refresh worker）
│   │   └── views/         # 页面组件（Dashboard, ItemList, Quests...）+ desktop/（Setup 等）
│   └── package.json
├── application/desktop/   # 桌面端 Tauri 2 外壳（src-tauri/ Rust + 复用 web/）
...
桌面端为**瘦客户端**：复用 `web/` 前端，经 HTTPS 直连后端，令牌存于系统钥匙串。**服务端零改动**。

- **预编译包**：见仓库 **Releases**（首个 macOS 版 `v1.0.0`，Apple Silicon）。
- **本地开发 / 自行构建**：见 [`application/desktop/README.md`](../../application/desktop/README.md)。

```bash
cd application/desktop
npm install                 # 装 @tauri-apps/cli（需 Rust 工具链 + Xcode CLT）
npm run dev                 # tauri dev：首屏接驳服务器 → 登录
npm run build               # tauri build → 产出 .dmg / .app
```

技术方案见 [docs/technology/260726-桌面端客户端技术方案.md](../../docs/technology/260726-桌面端客户端技术方案.md)。
...
| 文档 | 内容 |
|------|------|
| [docs/technology/260726-桌面端客户端技术方案.md](../../docs/technology/260726-桌面端客户端技术方案.md) | 桌面端 Tauri 2 瘦客户端：架构、平台抽象层、CSP/CORS、打包分发、移动端演进 |
| [docs/technology/260727-鉴权会话化与应用锁.md](../../docs/technology/260727-鉴权会话化与应用锁.md) | refresh 轮换 + 复用检测 + 设备会话 + 桌面应用锁（PIN/Touch ID）设计与实现 |
...
| 路径前缀 | 说明 |
|----------|------|
| `/api/auth` | 注册、登录、Token 刷新（轮换）、改密码、个人信息更新 |
| `/api/auth/sessions` | 设备会话列表、远程登出指定设备 |
